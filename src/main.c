...
static void draw_cell(uint8_t x, uint8_t y, char tile) {
    ...
    else if (tile == 'C') { textcolor(COLOR_WHITE); bgcolor(COLOR_RED); cputc('!'); }
    ...
    else if (tile == 'Q') { textcolor(COLOR_BLUE); bgcolor(COLOR_GRAY3); cputc('Q'); }
    else if (tile == 'V') { textcolor(COLOR_WHITE); bgcolor(COLOR_RED); cputc('V'); }
    else if (tile == 'R') { textcolor(COLOR_WHITE); bgcolor(COLOR_BROWN); cputc('R'); }
...
static void draw_hud(void) {
    clear_rows(20u, 29u);
    textcolor(COLOR_YELLOW); gotoxy(0u, 20u); cputs(" PROTOCOL 7: THE ASH LEDGER // V0.100");
...
static void draw_world(void) {
    uint8_t x, y;
    videomode(VIDEOMODE_40x30); bordercolor(COLOR_ORANGE); bgcolor(COLOR_BLACK); clrscr();
...
static void dialogue(const char* name, const char* one, const char* two, const char* three) {
    ...
    gotoxy(2u, 20u); revers(1); cputs(name); revers(0);
    textcolor(COLOR_WHITE); gotoxy(2u, 22u); cputs(one);
    gotoxy(2u, 24u); cputs(two);
    gotoxy(2u, 26u); cputs(three);
    textcolor(COLOR_GRAY3); gotoxy(8u, 28u); cputs("PRESS ANY KEY"); cgetc();
}
...
static uint8_t skill_scene(uint8_t who, const uint8_t* dice, const char* skill, uint8_t target) {
    ...
    else { textcolor(COLOR_LIGHTRED); cputs("FAILURE -- THE STORY MOVES FORWARD"); }
    textcolor(COLOR_GRAY3); gotoxy(2u, 19u); cputs("1-3 ARE BLANK. EVERY 4+ SCORES.");
    gotoxy(2u, 23u); cputs("PRESS ANY KEY."); cgetc();
...
static void draw_battle(uint8_t venn_hp, uint8_t doubt, uint8_t ap, uint8_t active) {
    uint8_t y;
    ...
    textcolor(COLOR_YELLOW); gotoxy(2u, 18u); cputs("[1-3] VECTOR  [A] ATTACK  [T] REASON");
    gotoxy(2u, 20u); cputs("[G] GUARD  [H] NANOGEL  [E] END TURN");
    clear_rows(22u, 28u);
}
...
static void battle_message(const char* label, uint8_t left, uint8_t right, uint8_t amount) {
    ...
    gotoxy(2u, 27u); cprintf("MARGIN %u -- PRESS ANY KEY", amount); cgetc();
}

static uint8_t confrontation(void) {
    ProtocolPool a_pool, v_pool; ProtocolResult a_roll, v_roll;
    uint8_t venn_hp = 26u, doubt = 0u, ap = 3u, active = 0u, guard = 0u;
    ...
    while (venn_hp && doubt < 12u && party_alive()) {
        ...
        draw_battle(venn_hp, doubt, ap, active); key = cgetc();
        ...
        if (key == 'e' || key == 'E') ap = 0u;
            ...
            battle_message("OPPOSED ATTACK", a_roll.total, v_roll.total, amount);
            ...
            battle_message("OPPOSED INFLUENCE", a_roll.total, v_roll.total, amount);
        ...
        if (!ap && venn_hp && doubt < 12u) {
            ...
            draw_battle(venn_hp, doubt, 0u, active); battle_message("VENN'S COUNTERATTACK", a_roll.total, v_roll.total, amount);
            ap = 3u; guard = 0u;
        }
    }
...
static void debug_state(void) {
    ...
    textcolor(COLOR_GRAY3); gotoxy(2u, 21u); cputs("PRESS ANY KEY."); cgetc();
}
...
static uint8_t title_screen(void) {
    ...
    textcolor(COLOR_LIGHTGREEN); gotoxy(8u, 22u); cputs("PRESS ANY KEY TO INSERT");
    textcolor(COLOR_GRAY2); gotoxy(13u, 27u); cputs("V0.100 PRE-ALPHA");
...
static void briefing(void) {
    ...
    gotoxy(9u, 27u); cputs("PRESS ANY KEY"); cgetc();
    ...
    textcolor(COLOR_GRAY3); gotoxy(9u, 25u); cputs("PRESS ANY KEY"); cgetc();
}
...
static void verification(void) {
    ...
    textcolor(COLOR_GRAY3); gotoxy(8u, 28u); cputs("END OF V0.100"); cgetc();
}

int main(void) {
    uint8_t action, nx, ny, joy_ok;
    protocol_seed(0x1906u); joy_ok = joy_install(joy_static_stddrv) == JOY_ERR_OK; upload_player_sprite();
    if (!title_screen()) return 0; briefing(); draw_world();
    while (party_alive() && !mission_done) {
        ...
        if (walkable(nx, ny)) {
            ...
            sprite_position(); sprite_image(walk_frame); draw_hud();
        }
        delay_frames(6u);
    }
